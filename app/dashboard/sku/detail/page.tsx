...
import { goods_detail_add_one, goods_detail_bycategoryId, goods_detail_delete, goods_detail_update, tmp_goods_detail_upload } from "@/app/service/sku/detail";
...
const App: React.FC = () => {
  ...
  const onFinish = async (values: any) => {
    ...
    await goods_detail_update(sData)
    const res = await goods_detail_bycategoryId(skuData)
 ...
 const onReset = () => {
  ...
  const onChange = async (id: string) => {
    ...
    setSkuData(specData)
    const res = await goods_detail_bycategoryId(specData)
    setTdata(res.data.result)
  };
  
  const handleAdd = async(id:string) => {
    // 调用数据库接口生成一行随机数据
    await goods_detail_add_one(id)
    // 更新渲染数据
    const res = await goods_detail_bycategoryId(skuData)
    setTdata(res.data.result)
  };
...
const handledelete = async(record:any) => {
  await goods_detail_delete(record.id)
  const res = await goods_detail_bycategoryId(skuData)
  setTdata(res.data.result)
};
    ...
    >
      ...
      <Upload
          ...
          customRequest={async ({ file, onSuccess, onError }) => {
            ...
            try {
              const response = await tmp_goods_detail_upload(formData)
